 ...
 */
const COOKIE_JAR_IIFE = "(()=>{var U=Object.defineProperty;var G=(e,t)=>{for(var r in t)U(e,r,{get:t[r],enumerable:!0})};var E={};G(E,{VirtualCookieJar:()=>D,canonicalHost:()=>L,cookieJarKey:()=>w,cookiePathMatches:()=>F,defaultPath:()=>z,domainMatches:()=>M,mergeCookieHeaders:()=>te,parseSetCookie:()=>K});var k={decodeValues:!0,map:!1,silent:!1,split:\"auto\"};function _(e){return typeof e!=\"string\"||e in{}}function N(){return Object.create(null)}function V(e){return typeof e==\"string\"&&!!e.trim()}function C(e,t){var r=e.split(\";\").filter(V),a=r.shift();if(!a)return null;var i=R(a),s=i.name,n=i.value;if(t=t?Object.assign({},k,t):k,_(s))return null;try{n=t.decodeValues?decodeURIComponent(n):n}catch(c){console.error(\"set-cookie-parser: failed to decode cookie value. Set options.decodeValues=false to disable decoding.\",c)}var o=N();return o.name=s,o.value=n,r.forEach(function(c){var m=c.split(\"=\"),f=m.shift().trim().toLowerCase();if(!_(f)){var d=m.join(\"=\").trim();if(f===\"expires\")o.expires=new Date(d);else if(f===\"max-age\"){var p=parseInt(d,10);Number.isNaN(p)||(o.maxAge=p)}else f===\"secure\"?o.secure=!0:f===\"httponly\"?o.httpOnly=!0:f===\"samesite\"?o.sameSite=d:f===\"partitioned\"?o.partitioned=!0:f&&(o[f]=d)}}),o}function R(e){var t=\"\",r=\"\",a=e.split(\"=\");return a.length>1?(t=a.shift(),r=a.join(\"=\")):r=e,{name:t,value:r}}function y(e,t){if(t=t?Object.assign({},k,t):k,!e)return t.map?N():[];if(e.headers)if(typeof e.headers.getSetCookie==\"function\")e=e.headers.getSetCookie();else if(e.headers[\"set-cookie\"])e=e.headers[\"set-cookie\"];else{var r=e.headers[Object.keys(e.headers).find(function(n){return n.toLowerCase()===\"set-cookie\"})];!r&&e.headers.cookie&&!t.silent&&console.warn(\"Warning: set-cookie-parser appears to have been called on a request object. It is designed to parse Set-Cookie headers from responses, not Cookie headers from requests. Set the option {silent: true} to suppress this warning.\"),e=r}var a=t.split,i=Array.isArray(e);if(a===\"auto\"&&(a=!i),i||(e=[e]),e=e.filter(V),a&&(e=e.map(A).flat()),t.map){var s=N();return e.reduce(function(n,o){var c=C(o,t);return c&&!_(c.name)&&(n[c.name]=c),n},s)}else return e.map(function(n){return C(n,t)}).filter(Boolean)}function A(e){if(Array.isArray(e))return e;if(typeof e!=\"string\")return[];var t=[],r=0,a,i,s,n,o;function c(){for(;r<e.length&&/\\s/.test(e.charAt(r));)r+=1;return r<e.length}function m(){return i=e.charAt(r),i!==\"=\"&&i!==\";\"&&i!==\",\"}for(;r<e.length;){for(a=r,o=!1;c();)if(i=e.charAt(r),i===\",\"){for(s=r,r+=1,c(),n=r;r<e.length&&m();)r+=1;r<e.length&&e.charAt(r)===\"=\"?(o=!0,r=n,t.push(e.substring(a,s)),a=r):r=s+1}else r+=1;(!o||r>=e.length)&&t.push(e.substring(a,e.length))}return t}y.parseSetCookie=y;y.parse=y;y.parseString=C;y.splitCookiesString=A;var J={maxNameValueBytes:4096,maxAttrValueBytes:1024,maxPerDomain:180,maxTotal:3e3,maxLifetimeMs:400*24*60*60*1e3,defaultSameSite:\"lax\",treatLocalhostAsSecure:!0,defaultHost:\"localhost\",isPublicSuffix:e=>!e.includes(\".\")},Q=new TextEncoder,$=e=>Q.encode(e).length;function w(e,t){return`${e}\\0${t}`}var X=e=>`${e.domain}\\0${e.path}\\0${e.name}`;function L(e){return e.trim().toLowerCase().replace(/^\\[|\\]$/g,\"\").replace(/\\.$/,\"\")}var Y=e=>/^\\d{1,3}(\\.\\d{1,3}){3}$/.test(e)||e.includes(\":\"),Z=e=>e===\"localhost\"||e.endsWith(\".localhost\")||e===\"::1\"||/^127\\./.test(e);function M(e,t){return e===t?!0:!Y(e)&&e.endsWith(\".\"+t)}function z(e){let t=e.split(\"?\")[0]||\"/\";if(!t.startsWith(\"/\"))return\"/\";let r=t.lastIndexOf(\"/\");return r<=0?\"/\":t.slice(0,r)}function F(e,t){return t===e||e.indexOf(t)===0&&(t.charAt(t.length-1)===\"/\"||e.charAt(t.length)===\"/\")}var j=(e,t)=>e.expires!==null&&e.expires<=t;function ee(e,t,r,a){return e===\"none\"||t?!0:e===\"lax\"?r&&a:!1}function K(e,t,r,a,i=Date.now()){let s=g=>({ok:!1,reason:g}),n=L(t.host),o=String(e).split(\";\"),c=o.shift()??\"\",m=c.indexOf(\"=\");if(m<0)return s(\"missing '=' in name-value pair\");let f=c.slice(0,m).trim(),d=c.slice(m+1).trim();if(!f)return s(\"empty cookie name\");if($(f)+$(d)>a.maxNameValueBytes)return s(`name+value exceeds ${a.maxNameValueBytes} bytes`);let p=null,u=null,l=!1,h=!1,T=null,W=null,H=null;for(let g of o){let b=g.indexOf(\"=\"),P=(b<0?g:g.slice(0,b)).trim().toLowerCase(),v=b<0?\"\":g.slice(b+1).trim();if(!($(v)>a.maxAttrValueBytes))switch(P){case\"path\":p=v.startsWith(\"/\")?v:null;break;case\"domain\":u=v.replace(/^\\./,\"\").toLowerCase()||null;break;case\"secure\":l=!0;break;case\"httponly\":h=!0;break;case\"samesite\":{let x=v.toLowerCase();T=x===\"strict\"||x===\"lax\"||x===\"none\"?x:null;break}case\"max-age\":if(/^-?\\d+$/.test(v)){let x=parseInt(v,10);W=x<=0?0:i+Math.min(x*1e3,a.maxLifetimeMs)}break;case\"expires\":{let x=Date.parse(v);Number.isNaN(x)||(H=Math.min(x,i+a.maxLifetimeMs));break}}}let I=n,O=!0;if(u)if(a.isPublicSuffix(u)){if(u!==n)return s(`Domain=${u} is a public suffix`)}else if(M(n,u))I=u,O=!1;else return s(`host \"${n}\" does not domain-match Domain=${u}`);let q=p??z(t.path??\"/\");if(l&&!r)return s(\"Secure cookie set from an insecure origin\");let S=T??a.defaultSameSite;if(S===\"none\"&&!l)return s(\"SameSite=None requires Secure\");if(S!==\"none\"&&t.sameSite===!1&&!t.topLevelNavigation)return s(`SameSite=${S} cookie set from a cross-site response`);let B=f.toLowerCase();return B.startsWith(\"__secure-\")&&!l?s(\"__Secure- prefix requires Secure\"):B.startsWith(\"__host-\")&&(!l||!O||q!==\"/\")?s(\"__Host- prefix requires Secure, no Domain, and Path=/\"):{ok:!0,cookie:{name:f,value:d,domain:I,hostOnly:O,path:q,secure:l,httpOnly:h,sameSite:S,expires:W??H,created:i,lastAccessed:i}}}function te(e,t){if(!t)return e||\"\";if(!e)return t;let r=new Set,a=[];for(let i of t.split(\";\")){let s=i.trim();if(!s)continue;let n=s.indexOf(\"=\");r.add((n<0?s:s.slice(0,n)).trim()),a.push(s)}for(let i of e.split(\";\")){let s=i.trim();if(!s)continue;let n=s.indexOf(\"=\");r.has((n<0?s:s.slice(0,n)).trim())||a.push(s)}return a.join(\"; \")}var D=class{_jars=new Map;cfg;constructor(t={}){this.cfg={...J,...t}}isSecureOrigin(t){return!!t.secure||this.cfg.treatLocalhostAsSecure&&Z(L(t.host))}store(t,r,a,i={host:this.cfg.defaultHost}){let s=[];if(a==null)return s;let n=Array.isArray(a)?a.flatMap(d=>A(d)):A(a),o=w(t,r),c=this._jars.get(o);c||(c=new Map,this._jars.set(o,c));let m=Date.now(),f=this.isSecureOrigin(i);for(let d of n){let p=K(d,i,f,this.cfg,m);if(!p.ok){s.push({raw:d,reason:p.reason});continue}let u=p.cookie,l=X(u);if(j(u,m)){c.delete(l);continue}let h=c.get(l);h&&(u.created=h.created),c.set(l,u)}return this.enforceLimits(c,m),c.size===0&&this._jars.delete(o),s}enforceLimits(t,r){for(let[s,n]of t)j(n,r)&&t.delete(s);let a=new Map;for(let s of t){let n=a.get(s[1].domain);n?n.push(s):a.set(s[1].domain,[s])}for(let s of a.values()){let n=s.length-this.cfg.maxPerDomain;if(!(n<=0)){s.sort((o,c)=>o[1].lastAccessed-c[1].lastAccessed);for(let[o]of s.slice(0,n))t.delete(o)}}let i=t.size-this.cfg.maxTotal;if(i>0){let s=[...t].sort((n,o)=>n[1].lastAccessed-o[1].lastAccessed);for(let[n]of s.slice(0,i))t.delete(n)}}cookieHeader(t,r,a){let i=this._jars.get(w(t,r));if(!i||i.size===0)return\"\";let s=typeof a==\"string\"?{host:this.cfg.defaultHost,path:a}:a,n=Date.now(),o=L(s.host),c=(s.path??\"/\").split(\"?\")[0]||\"/\",m=this.isSecureOrigin(s),f=s.sameSite??!0,d=!!s.topLevelNavigation,p=[\"GET\",\"HEAD\",\"OPTIONS\",\"TRACE\"].includes((s.method??\"GET\").toUpperCase()),u=[];for(let[l,h]of i){if(j(h,n)){i.delete(l);continue}(h.hostOnly?h.domain!==o:!M(o,h.domain))||F(c,h.path)&&(h.secure&&!m||s.script&&h.httpOnly||ee(h.sameSite,f,d,p)&&u.push(h))}u.sort((l,h)=>h.path.length-l.path.length||l.created-h.created);for(let l of u)l.lastAccessed=n;return u.map(l=>`${l.name}=${l.value}`).join(\"; \")}list(t,r){let a=this._jars.get(w(t,r));return a?[...a.values()].map(i=>({...i})):[]}clearInstance(t){for(let r of[...this._jars.keys()])r.startsWith(t+\"\\0\")&&this._jars.delete(r)}clearAll(){this._jars.clear()}};globalThis.__cookieJarLib=E;})();";
