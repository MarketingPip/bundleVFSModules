...
const REPORTERS = { dot, spec, tap, junit, lcov };

/** Currently active reporter — defaults to spec, overridable per-run. */
let _activeReporter = spec;
...
export function _reset() {
  ...
  _activeReporter = spec; // reset to default reporter
...
function makeApiValues() {
  ...
  return [test, it, suite, describe, before, after, beforeEach, afterEach, mock, snapshot, _assert, dot, spec, tap, junit, lcov];
}
...
globalThis._RUNTIME_TEST_RUNNER_ = {execute, _activeReporter};
