...
export class TLSSocket extends Socket {
  authorized = false  // false until peer cert verified — matches Node.js default
  encrypted = true
  ...
  getPeerCertificate(_detailed) {
    // Return empty object when no peer cert is available (no real TLS handshake)
    return {}
  }
  getCipher() {
    // Return null — no real cipher negotiated in a browser shim
    return null
  }
  getProtocol() {
    // Return null — no real TLS handshake performed
    return null
  }
  setServername(name) {
...
export class Server extends EventEmitter {
  ...
  address() {
    // Return null — no real bound address in a browser shim
    return null
  }
  getTicketKeys() {
