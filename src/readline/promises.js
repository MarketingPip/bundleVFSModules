// src/readline/promises.js
//
// Faithful port of Node.js v24.20.0:
//   - lib/readline/promises.js            (promises Interface)
//   - lib/internal/readline/promises.js   (Readline output controller)
//   - lib/internal/readline/interface.js  (Interface implementation, inlined)
//   - lib/internal/readline/utils.js      (errors, validators, CSI, inlined)
//
// Self-contained: the implementation is inlined here so this entry has no
 ...
 */
// NOTE: kept local-only — real node:readline/promises does not export emitKeys.
...
// ---------------------------------------------------------------------------
// Interface — promise-based question()
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Readline — output controller (port of internal/readline/promises.js)
//
// Queues cursor/screen operations against a Writable stream; commit() flushes
...
export class Readline {
  ...
  clearScreenDown() {
    if (this.#autoCommit) {
      nextTick(() => this.#stream.write(kClearScreenDown));
    } else {
...
// ESM default export, matching real Node: the same object as require() returns.
const readlinePromises = { Interface, Readline, createInterface };
